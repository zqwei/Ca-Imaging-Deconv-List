...
% record list for dff and cell names
pdff_list = []; % peak df/f
...
for ncell = 1:length(cells)
    ...
    % recorded variable
    % 1-6: nAP peak df/f, n = 0-5 respectively
    amplitude=nan(1, 6);
    for k=1:6
        df_f=rawf2df_f(s(k).ca_waveform,1:10);
        % df_f  = s(k).ca_waveform;
        if size(df_f, 2)>0
            amplitude(k) = mean(max(df_f(1:56,:), [],1));
        end
    end
    pdff_list = [pdff_list; amplitude]; %#ok<AGROW>
end

%average over recording session for the same cell
pdff_list_ = grpstats(pdff_list, cell_list, 'nanmean'); 

% plot -- Figure 3G
figure;
hold on
mean_ = nanmedian(pdff_list_);
bar(0:5, mean_, 'facecolor', 'none');
plot(0:5, pdff_list_, '-ok')
xlim([-0.5, 5.5])
% ylim([0, 3])
box off
setPrint(8, 6, [PlotDir 'peak_distribution_6s_TG'], 'pdf')